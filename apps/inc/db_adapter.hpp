...
class DbClient {
    public:
        DbClient(std::string uri);
        DbClient();
        ~DbClient();

        std::string get_database() const { return(mdbName); }
        void set_database(std::string dbName) { mdbName = dbName; }
        std::string get_uri() const { return(mURI); }
        void set_uri(std::string uri) { mURI = uri; }
        ...
        std::int32_t create_bulk_documentEx(std::string collectionName, std::string doc);
    ...
    private:
        ...
        std::string mdbName;
#ifdef IOT_ENABLE_MONGO
        std::unique_ptr<mongocxx::pool> mConnPool;
