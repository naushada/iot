...
ResourceMap load_object_resources(const std::string& path) {
    ...
    if (!L) {
        ACE_ERROR((LM_ERROR,
                   ACE_TEXT("%D lwm2m:thread:%t %M %N:%l lua_config %C: luaL_newstate failed\n"),
                   path.c_str()));
        return out;
    }
    luaL_openlibs(L.get());
    ...
    if (luaL_dofile(L.get(), path.c_str()) != LUA_OK) {
        ...
        if (msg.find("cannot open") == std::string::npos) {
            ACE_ERROR((LM_ERROR,
                       ACE_TEXT("%D lwm2m:thread:%t %M %N:%l lua_config %C: %C\n"),
    ...
    if (!lua_istable(L.get(), -1)) {
        ACE_ERROR((LM_ERROR,
                   ACE_TEXT("%D lwm2m:thread:%t %M %N:%l lua_config %C: top-level return is not a table\n"),
                   path.c_str()));
        return out;
    }
    ...
    if (lua_next(L.get(), -2) == 0) {
        ACE_ERROR((LM_WARNING,
                   ACE_TEXT("%D lwm2m:thread:%t %M %N:%l lua_config %C: top-level table is empty\n"),
                   path.c_str()));
        return out;
    }
    ...
    if (!lua_istable(L.get(), -1)) {
        ACE_ERROR((LM_ERROR,
                   ACE_TEXT("%D lwm2m:thread:%t %M %N:%l lua_config %C: object value is not a table\n"),
                   path.c_str()));
        lua_pop(L.get(), 2);
        return out;
    }
