...
| `iot.bs.psk.override` | boolean | `write_acl={"gid:engineer"}` (readable), default `false` | Third-party BS toggle (see below). When `true` the client uses `iot.bs.psk.identity` **verbatim** instead of `sha256(endpoint)`. Non-secret mode flag (like `iot.dev.mode`): write-gated but readable so the UI reflects true state. |

### Third-party bootstrap server override (`iot.bs.psk.override`)

The default identity model derives the on-the-wire BS PSK identity as
`sha256(endpoint)[:32]` — the device and our cloud BS both compute it, so the
identity is never commissioned. A **third-party** bootstrap server mints its
own identity/PSK pair that will *not* match that derivation. For that case:

- Set `iot.bs.psk.override = true` and provision `iot.bs.psk.identity` (the
  vendor's identity string, used verbatim) + `iot.bs.psk.key` (the vendor's
  hex PSK, same write-only key as the default flow).
- The client (`main.cpp`, BS credential setup) reads the override flag: on →
  `identity = iot.bs.psk.identity`; off → `identity = sha256(endpoint)[:32]`
  (unchanged legacy behaviour).
- `DsConfig::set_serial()` skips its auto-write of `iot.bs.psk.identity` while
  the override is on, so the RPi serial auto-fill can't clobber the operator's
  value.
- **DM is unaffected** — DM credentials still arrive via the bootstrap commit
  and switch in through `active_identity()`. No `iot.dm.*` key or code path
  changes.
- device-ui exposes this on the **Security (Bootstrap)** tab, in the existing
  dev-mode + Admin "Bootstrap PSK" card: a *"Use custom Bootstrap PSK
