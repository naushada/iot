...
| BUG-009 | ~~`cli::split` spins forever when the input starts with the delimiter (e.g. `read path=/3/0/0`). `istream::get(streambuf, delim)` sets `failbit` on zero-char extraction; the subsequent `iss.get()` refuses to consume because of the failbit; loop never makes progress. The legacy `Readline::str2Vector` (where this code was lifted from) hid the bug because callers wrote `uri="/push"` quoted — the leading `"` shifted the first iteration off the delim. The new CLI parser doesn't strip quotes, so every LwM2M-path command after `register` wedged the binary at 198 % CPU.~~ **FIXED 2026-05-30** (commit `7f8bbb7`, mislabelled "BUG-003" in the commit message — the canonical id is BUG-009): replaced the stream-based split in `apps/src/cli/coap_dispatch.cpp::split` with a plain character loop that handles leading/trailing/consecutive delims correctly and still strips quote characters for legacy compatibility. Smoke regression: `log/L9/run-cli-smoke.sh` → 20-frame `cli-smoke.pcap` covering every command. | smoke test (L9) | M |
| BUG-010 | ~~Post-quit segfault: `UDPAdapter::stop()` ended the reactor loop and joined the worker thread without first cancelling the periodic 1 Hz timer or removing the `READ_MASK` handlers; the reactor singleton's later teardown then dereferenced freed `ServiceContext_t` pointers.~~ **FIXED 2026-05-30** (commit `55c5f32`, originally tracked as FUP-5). Fix: `stop()` guards against double-stop, cancels the timer, removes every service handler with `DONT_CALL`, then ends the reactor loop. | smoke test (L9) | M |
...
| NFR-INTEROP-002 | Our Server SHALL pass the equivalent test with Leshan as client. **Plain-CoAP closed 2026-05-29 with the wakaama lwm2mclient (Leshan-equivalent compliant client; no canonical Leshan client image on Docker Hub). 4-frame Register + Update round-trip verified on the wire (`log/L9/nfr-002-coap.pcap`, 482 B, commit 8ca19c1). REQ-REG-001/002/003/005/007/009 all green; wakaama held STATE_READY for the full window. DTLS/PSK variant attempted 2026-05-29 with a locally-built `wakaama-dtls` image (Docker Hub ships CoAP-only wakaama): wakaama's ClientHello reaches our server and is parsed, but no HelloVerifyRequest is emitted by our tinydtls 0.8.6 — handshake stalls, Register never sent (`log/L9/nfr-002-dtls.pcap`, 165 B, 1 frame, commit 292a848). Filed as FUP-4 — see `log/L9/results.md` → "NFR-INTEROP-002 — DTLS/PSK variant".** | M |
...
| L8 — Device object live values + Conn-Mon | REQ-OBJ-003/005 (closed via apps/inc/lwm2m_object_3_device.hpp + apps/inc/lwm2m_object_stubs.hpp + apps/config/deviceObject/0.lua) | Manual smoke against a real Linux host. |
...
| 2026-05-29 | D6 | **Device Object (OID 3) reads from `apps/config/deviceObject/0.lua` with hard-coded constants as fallback.** (Was JSON at decision time — switched to Lua on 2026-05-30, commit 1d92b66, to align with grace-server's config loader; the legacy `.json` files were removed on 2026-05-30.) On startup the installer attempts to parse the Lua chunk via `iot::lua_config::load_object_resources`; on missing file, parse error, or per-RID type mismatch, the corresponding RID falls back to its compiled-in default (Manufacturer="Sierra Wireless", Model="LwM2M Client", Firmware="0.1", etc.). Per-RID granularity means a Lua file that overrides only Manufacturer leaves all other RIDs at their defaults. Live RIDs (Memory Free / Total / Current Time) still bind to platform readers regardless of Lua content. | Tunable per deployment without a rebuild, but the binary still boots clean on a fresh host. Matches the existing security/server-object pattern so operators have one consistent config shape to learn. | REQ-OBJ-003, REQ-OBJ-005 |
