...
int Server::open() {
    ...
    if (::unlink(m_socketPath.c_str()) == -1 && errno != ENOENT) {
        ACE_ERROR_RETURN((LM_ERROR,
                          ACE_TEXT("%D dsserver:thread:%t %M %N:%l unlink(%C) "
                                   "failed errno=%d\n"),
    ...
    if (m_acceptor.open(addr) == -1) {
        ACE_ERROR_RETURN((LM_ERROR,
                          ACE_TEXT("%D dsserver:thread:%t %M %N:%l acceptor.open(%C) "
                                   "failed errno=%d\n"),
                          m_socketPath.c_str(), errno),
                         -1);
    }
    ...
    if (::chmod(m_socketPath.c_str(), kDefaultMode) == -1) {
        ACE_ERROR((LM_WARNING,
                   ACE_TEXT("%D dsserver:thread:%t %M %N:%l chmod(%C, 0%o) "
    ...
    if (ACE_Reactor::instance()->register_handler(
        ...
        ACE_ERROR((LM_ERROR,
                   ACE_TEXT("%D dsserver:thread:%t %M %N:%l register_handler "
                            "failed errno=%d\n"),
                   errno));
        m_acceptor.close();
    ...
    ACE_DEBUG((LM_DEBUG,
               ACE_TEXT("%D dsserver:thread:%t %M %N:%l listening on %C (mode 0%o)\n"),
               m_socketPath.c_str(),
...
int Server::handle_input(ACE_HANDLE /*fd*/) {
    ...
    if (m_acceptor.accept(stream) == -1) {
        ACE_ERROR_RETURN((LM_ERROR,
                          ACE_TEXT("%D dsserver:thread:%t %M %N:%l accept "
                                   "failed errno=%d\n"),
                          errno),
                         0);     // stay registered
    }
    ...
    if (!w) {
        ACE_ERROR((LM_ERROR,
                   ACE_TEXT("%D dsserver:thread:%t %M %N:%l no worker pool; "
                            "dropping connection\n")));
        stream.close();
        return 0;
    }
    ...
    if (ACE_Reactor::instance()->register_handler(
        ...
        ACE_ERROR((LM_ERROR,
                   ACE_TEXT("%D dsserver:thread:%t %M %N:%l register_handler for "
