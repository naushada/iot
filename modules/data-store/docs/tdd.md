...
| REQ-DS-020 | `Client::connect()` MUST spawn an internal listener thread that owns the read side of the socket and demuxes lines into pending-request promises (by `id`), the pull-style event queue, and per-watch callbacks. Synchronous methods (`set`/`get`/`watch`/`unwatch`) MUST stay synchronous from the caller's view. | M |
| REQ-DS-021 | `Client::watch(keys, EventCallback)` MUST return a unique `WatchHandle`. The same Client MUST support N concurrent callback registrations with overlapping or distinct key sets; every callback subscribed to a changed key MUST fire on each event. | M |
| REQ-DS-022 | Per-key local refcount MUST gate wire-level `register` / `remove`: only the 0→1 transition emits `register`, only the N→0 transition emits `remove`. | M |
| REQ-DS-023 | `ds-server` MUST accept a `ds-schema-dir=PATH` flag. At startup it scans the directory for `*.lua` schema files; each entry adds a `(type, default, min?, max?)` spec keyed by full key name. Duplicates across files log a WARNING (last-loaded wins). Set with a schema-known key that fails type/range validation MUST be rejected with `{ok:false, err:"schema(K): …"}`. Get on an absent key with a schema default MUST return that default. Unknown keys MUST pass through (schema is opt-in). | M |
| REQ-DS-024 | Server-side diagnostic logging MUST go through the ACE logging API (`ACE_DEBUG` / `ACE_ERROR`), not `std::cout` / `std::cerr`. CLI user-facing output (`ds-cli`) is exempt — its stdout is the contract with the operator/scripts. | M |
...
| 2026-05-31 | DS-D8 | **Single Client owns a listener thread**, demuxing every line into per-request promises, the pull queue, and per-watch callbacks. Sync methods still look sync; events fire on the listener. Pull + callback styles coexist on one Client. | App code is dramatically cleaner than the "two clients" or "app-side reactor" alternatives. The cost is one std::thread per Client — acceptable for our deployment shape (apps have few connections). | §13, REQ-DS-020/021/022 |
| 2026-05-31 | DS-D9 | **Per-client Lua schema files.** `ds-server` reads every `*.lua` under `ds-schema-dir/` into an immutable SchemaRegistry. Set validates type+range; get falls back to schema default; unknown keys pass through. Multi-file design = one schema per client/user; duplicates WARN + last-loaded-wins. | Operators wanted typed, owner-scoped keys without changing the wire protocol. Per-file ownership matches how the rest of the iot config tree works (one Lua file per object instance under apps/config/). | §14, REQ-DS-023 |
| 2026-05-31 | DS-D10 | **Server-side diagnostic logging uses ACE.** All `std::cout`/`std::cerr` in `src/server/` swapped to `ACE_DEBUG`/`ACE_ERROR`. ds-cli stays on stdout/stderr (its output is the operator/script contract). | Matches the iot binary's logging discipline — one timestamp/thread format across modules, one place to redirect (file/syslog) when the operator decides. | REQ-DS-024 |
...
|-------|-------------------------------------------------------------------|-----------------------------------------------------------------|------------------------------------------------------------------------------------------------------------|--------|
| ~~**D1**~~ | Module scaffold + `ds-server` (LSOCK acceptor → welcome → close) + `libdatastore_client.a` + `ds-cli`. | REQ-DS-011, REQ-DS-018, REQ-DS-019, NFR-DS-004                  | `DS_REQ_DS_011_concurrent_sessions` (16 parallel clients) ✓ + `DS_NFR_DS_004_socket_mode_is_0660` ✓ | **Closed** 2026-05-30 (`650120e`) |
| **D1.5** *(new)* | Active-object worker pool (5 ACE_Task workers, round-robin from acceptor). | NFR — reactor thread never blocks on per-request handler.       | `WorkerPool.round_robin_visits_every_worker_then_wraps` ✓                          | **Closed** 2026-05-30 (`6d4c37c`) — and POSIX → ACE in client lib `8800b2f` |
| ~~**D2**~~ | Line-delimited JSON parser + dispatch; `ok=false,err=…` for unknown ops; vectorised arg shape parsed.                                  | REQ-DS-010, REQ-DS-012, REQ-DS-014                              | Landed combined with D3+D5 — protocol surface exercised end-to-end by all three Protocol tests in `protocol_test.cpp`. | **Closed** 2026-05-30 (`51c92f3`) |
| ~~**D3**~~ | In-memory `DataStore` with get/set/remove; protocol wires up; no persistence yet.                                                       | REQ-DS-003                                                      | `Protocol.DS_REQ_DS_003_set_then_get_returns_latest` ✓ (vector get, missing key → null).                                | **Closed** 2026-05-30 (`51c92f3`) |
| ~~**D5**~~ | Notification engine: register / remove / fan-out / unchanged-skip; per-session subscription set.                                        | REQ-DS-004..009                                                 | `Protocol.DS_REQ_DS_007_register_then_set_emits_notify_to_watcher` ✓ + `Protocol.DS_REQ_DS_006_unchanged_value_no_notify` ✓ | **Closed** 2026-05-30 (`51c92f3`) |
| ~~**D4**~~ | LuaPersistor: load at startup, write-through on set/remove; corrupted-state exit.                                                       | REQ-DS-001, REQ-DS-002, REQ-DS-015, REQ-DS-016, NFR-DS-001/003 | `LuaPersistor.*` + `DataStorePersist.set_flushes_to_disk_and_load_restores` ✓ + `log/L10/run-ds-persist-smoke.sh` end-to-end restart smoke ✓ | **Closed** 2026-05-30 (`f1fda73`) |
| ~~**D6**~~ | `ds-cli` test client + `log/L10/run-ds-smoke.sh` heredoc-driven smoke harness.                       | NFR-DS-008                                                      | `log/L10/ds-smoke.txt` captures welcome → set → get → register-from-watcher → set-from-setter → changed event → unwatch on the wire. | **Closed** 2026-05-30 (`33d54e1`) |
| ~~**D7**~~ | Docs: update `architecture.md`, add `Related docs` cross-links, mark RDD rows closed, write `log/L10/results.md` mirroring L9 style.    | —                                                              | Doc-only; no test.                                                                                         | **Closed** 2026-05-30 (this commit) |
| ~~**D8**~~ | Client listener thread + promise demux + per-watch `EventCallback` registry + `WatchHandle` API. Pull-style `recv_event` preserved.    | REQ-DS-020, REQ-DS-021, REQ-DS-022                              | `Callback.single_watch_with_callback_fires_on_event` ✓ + `Callback.multiple_watches_with_overlapping_keys_all_fire` ✓ (ref-counted unwatch). | **Closed** 2026-05-31 (`1fb4909`) |
| ~~**D9**~~ | `ds-schema-dir=PATH` flag → SchemaRegistry per-client Lua files; validate set; default-on-get; ACE WARNING on duplicate keys.           | REQ-DS-023                                                      | `Schema.*` (6 unit tests) + manual wire smoke (`set` rejection + `get` default).                          | **Closed** 2026-05-31 (`25a4e4b`) |
| ~~**D10**~~ | Sweep server-side `std::cout`/`std::cerr` → `ACE_DEBUG`/`ACE_ERROR`. ds-cli stays on stdout/stderr.                                    | REQ-DS-024                                                      | 30/30 tests still green; manual `ds-server` log inspection shows ACE format.                              | **Closed** 2026-05-31 (this commit) |
