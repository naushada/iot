...
#define DTLS_CH_LENGTH_MAX sizeof(dtls_client_hello_t) + DTLS_COOKIE_LENGTH_MAX + 12 + 26 + sizeof(session_id_t)
...
/**
 * Recalculate key block using the previously calculated master-secret
 */
static int
recalculate_key_block(dtls_context_t *ctx,
                      dtls_handshake_parameters_t *handshake,
                      dtls_peer_t *peer,
                      session_t *session,
                      dtls_peer_type role) {
  ...
  dtls_prf(handshake->tmp.master_secret,
           DTLS_MASTER_SECRET_LENGTH,
           PRF_LABEL(key), PRF_LABEL_SIZE(key),
           handshake->tmp.random.server, DTLS_RANDOM_LENGTH,
           handshake->tmp.random.client, DTLS_RANDOM_LENGTH,
...
dtls_send_server_hello(dtls_context_t *ctx, dtls_peer_t *peer)
{
  ...
  if (!peer)
  {
    dtls_debug("dtls_send_server_hello: undefined peer\n");
    return dtls_alert_fatal_create(DTLS_ALERT_INTERNAL_ERROR);
  }
  ...
  if (peer->session_id.size == 0) {
    ...
    dtls_int_to_uint8(p, peer->session_id.size);
    p += sizeof(uint8);
    memcpy(p, peer->session_id.id, peer->session_id.size);
    p += peer->session_id.size;
  }
...
dtls_send_client_hello(dtls_context_t *ctx, dtls_peer_t *peer,
  ...
  if (!peer)
  {
    dtls_debug("dtls_send_client_hello: undefined peer\n");
    return dtls_alert_fatal_create(DTLS_ALERT_INTERNAL_ERROR);
  }
		      ...
		      uint8 *data, size_t data_length)
{
  ...
  /* session id: the first byte represents the length of the session ID.
   * The following bytes represents the session ID.
   * When the server accepts to resume the previous session, it sends back
   * the session ID length and the session ID in order to notify the client.
  */
  if (*data) {
    ...
    if (peer->resumption) {
      ...
      memcpy(peer->session_id.id, data + 1, peer->session_id.size);
      data += peer->session_id.size + 1;
      data_length -= (peer->session_id.size + 1);
    }
  }
  else {
    SKIP_VAR_FIELD(data, data_length, uint8); /* skip session id */
...
int
dtls_resume(dtls_context_t *ctx, const session_t *dst)
{
  dtls_peer_t *peer = NULL;
  int err;
  ...
  if (!peer) {
    dtls_warn("peer not found\n");
    return -1;
  }

  if (peer->state != DTLS_STATE_CONNECTED) {
    dtls_warn("dtls not connected (%d)\n", peer->state);
    return -1;
  }

  if (!peer->handshake_params) {
    dtls_warn("last session handshake params not found\n");
    return -1;
  }
  ...
  if (peer->role == DTLS_CLIENT) {
    ...
    if (err < 0)
      ...
      peer->resumption = true;
      peer->state = DTLS_STATE_CLIENTHELLO;
    }
    return err;
  } else if (peer->role == DTLS_SERVER) {
    dtls_warn("resume not implemented on server side\n");
    return -1;
  }
...
handle_handshake_msg(dtls_context_t *ctx, dtls_peer_t *peer, session_t *session,
  ...
  case DTLS_HT_SERVER_HELLO:
    ...
    if (peer->resumption == true)
      peer->state = DTLS_STATE_WAIT_CHANGECIPHERSPEC;
    else if (is_tls_ecdhe_ecdsa_with_aes_128_ccm_8(peer->handshake_params->cipher))
  ...
  case DTLS_HT_FINISHED:
    ...
    if (role == DTLS_SERVER) {
    ...
    else if (peer->resumption) {
      peer->resumption = false;
      /* send ServerFinished */
      update_hs_hash(peer, data, data_length);

      dtls_security_params_switch_back(peer);
      /* send change cipher spec message and switch to new configuration */
      err = dtls_send_ccs(ctx, peer);
      if (err < 0) {
        dtls_warn("cannot send CCS message\n");
      ...
      err = dtls_send_finished(ctx, peer, PRF_LABEL(client), PRF_LABEL_SIZE(client));
      if (err < 0) {
        dtls_warn("sending server Finished failed\n");
        return err;
      }
...
dtls_handle_message(dtls_context_t *ctx,
  ...
  while (1) {
    ...
    case DTLS_CT_CHANGE_CIPHER_SPEC:
      ...
      /* In session resuming, after we receive ChangeCpherSpec form server,
       * we generate the new keys using old master secret as premaster secret,
       * server's FINISHED message will be encrypted using new keys,
       * but client's ChangeCpherSpec message will be encrypted using old keys.
       */
      if (peer && peer->resumption) {
        dtls_handshake_parameters_t *handshake = peer->handshake_params;
        recalculate_key_block(ctx, handshake, peer, &peer->session, peer->role);
        dtls_security_params_switch(peer);
      }
      break;
