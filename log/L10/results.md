...
| FUP-DS-9   | ✅ **Partial — lifetime apply done** (PR #16). `RegistrationClient::set_lifetime()` is lock-free (atomic uint32_t) so the listener thread can store a new value and the reactor thread reads it on its next tick. main.cpp registers a `ds.on_change` handler that calls `reg->set_lifetime` on `Key::Lifetime` and logs `applied iot.lifetime=N`. Endpoint + server.uri still log-only (no apply). |
| FUP-DS-10  | ✅ **Done** — `RegistrationClient::set_endpoint(s)` updates an internal mutex-guarded mirror + flips `pending_reregister()`. The 1Hz tick consumes the flag when state==Registered, sends Deregister (DELETE /rd/{loc}), then the existing Unregistered→Register branch fires a fresh Register with the new ep=. Idempotent: re-setting the same endpoint doesn't flag. 5 new unit tests in `apps/test/registration_client_test.cpp`. |
| FUP-DS-11  | ✅ **Done (plain CoAP)** — `ClientPlumbing::Rebind` mailbox fills from a Key::ServerUri change; tick consumes when state==Unregistered, swaps `ServiceContext_t::peerHost/peerPort` on the LwM2MClient service, then existing Unregistered → Register branch fires Register to the new server. `request_reregister()` added to RegistrationClient so a non-endpoint event can still trigger the cycle. Schema (`iot.server.uri = type=string`) catches malformed values at set time. CoAPs rebind logs LM_WARNING + applies the peer change anyway — DTLS teardown + reconnect remain (split as FUP-DS-12 only if a CoAPs deployment surfaces). |
