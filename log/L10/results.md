...
| FUP-DS-6   | **Ship + enforce `iot.*` schema.** `modules/data-store/schemas/iot.lua` already declares `iot.endpoint`/`iot.lifetime`/`iot.binding`/`iot.identity`/`iot.observable`; add `iot.server.uri`, ensure ds-server loads it by default (or document `ds-schema-dir=` placement), demonstrate rejection of `ds-cli set iot.lifetime -1` and `ds-cli set iot.endpoint 42`. Small (~30 min). |
| FUP-DS-7   | **iot binary watches its config keys.** Today it does one-shot reads at startup; switch to a `Client::watch` for `iot.endpoint`/`iot.lifetime`/`iot.server.uri` so `ds-cli set ...` from an operator hot-applies without restart. Requires picking the apply policy (full-reconnect vs incremental); blocked by FUP-DS-6 so the watched keys have a schema. Medium (~few hrs). |
